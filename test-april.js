...
async function main() {
  ...
  for (const r of rows) {
    ...
    if (isDhoniOrAll && cn.includes('sofa')) {
      console.log(`Dhoni/All with Sofa in CN -> LC: ${lc} | Camp: ${r.campaign_name}`);
    }
    
    if (isDhoniOrAll && an.includes('sofa')) {
      console.log(`Dhoni/All with Sofa in Adset -> LC: ${lc} | Adset: ${r.adset_name}`);
    }
  }
}
