...
async function run() {
  ...
  for (const row of geoMtdRes) {
    ...
    if (c.includes('mumbai') || c.includes('thane,') ||
      ...
      c.includes('nala sopara')) mappedTo = 'Mumbai';
        ...
        c.includes('kudlu')) mappedTo = 'Bengaluru';


    if (mappedTo === 'Bengaluru') {
        bngMapped.set(c, (bngMapped.get(c) || 0) + cost);
    } else if (!mappedTo && c.includes('maharashtra')) {
        maharashtraNull.set(c, (maharashtraNull.get(c) || 0) + cost);
    }
  }
  ...
  console.log("=== MAHARASHTRA NULL ===");
  for (const [c, cost] of Array.from(maharashtraNull.entries()).sort((a,b)=>b[1]-a[1])) {
      console.log(`${c}: ₹${cost.toFixed(2)}`);
  }
}
