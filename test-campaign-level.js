...
async function main() {
  const url = `${BASE}/${accountId}/insights?fields=campaign_name,spend,impressions,actions,action_values&time_increment=monthly&time_range={"since":"2026-06-01","until":"2026-06-30"}&level=campaign&limit=500&access_token=${access_token}`;
  ...
  for (const r of rows) {
    ...
    if (cn.includes('asc_chair_top_funnel_core_nst')) {
        ...
        const lp = parseInt(actions.find((a) => a.action_type === 'landing_page_view')?.value || '0', 10);
