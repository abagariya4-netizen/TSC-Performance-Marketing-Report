...
async function main() {
  ...
  for (const r of rows) {
    ...
    if (cn.includes('group')) {
       if (matchesCategoryForMetrics(r.campaign_name, r.adset_name, 'Mattress')) mattressRows.push(r.campaign_name + " | " + r.adset_name + " | " + r.spend);
       if (matchesCategoryForMetrics(r.campaign_name, r.adset_name, 'All')) allRows.push(r.campaign_name + " | " + r.adset_name + " | " + r.spend);
    }
  }

  console.log("Mattress ONLY:");
  for (let m of mattressRows) {
     if (!allRows.includes(m)) console.log(m);
  }
}
