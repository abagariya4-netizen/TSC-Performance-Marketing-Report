...
export function parseRegionPlanCSV(text: string): Record<string, number> {
  ...
  dataRows.forEach(row => {
    row.forEach((cell, i) => {
      if (KNOWN_STATES.includes(cell.toLowerCase().trim())) rScore[i]++;
  ...
  for (const row of allRows) {
    ...
    if (isCity) {
      currentCity = col0.includes('delhi')
        ? 'Delhi+NCR'
        : row[0].trim().split(' ').map((w:string) => w[0].toUpperCase() + w.slice(1).toLowerCase()).join(' ');
