...
async function run() {
  ...
  const mumbaiKeywords = ['mumbai', 'thane', 'kalyan', 'dombivali', 'dombivli', 'vasai', 'virar', 'mira bhayandar', 'bhiwandi', 'ambernath', 'ulhasnagar', 'panvel'];
  const bengaluruKeywords = ['bengaluru', 'bangalore', 'koramangala', 'indiranagar', 'whitefield', 'jayanagar', 'marathahalli', 'malleswaram', 'electronic city', 'hsr layout', 'bellandur', 'btm layout', 'kr puram', 'yelahanka', 'banashankari', 'hebbal', 'domlur', 'bommanahalli', 'halasuru', 'basavanagudi', 'kengeri', 'peenya', 'sarjapur', 'cv raman nagar', 'vijayanagar', 'mathikere', 'mahadevapura', 'rr nagar', 'rajajinagar', 'chikkakannalli', 'subramanyapura', 'narayanapura', 'indirapuram', 'nijagal', 'kudlu'];

  console.log("=== DOUBLE MATCHES ===");
  for (const [c, cost] of costs.entries()) {
      let isMum = false;
      let isBng = false;
      for (const k of mumbaiKeywords) if (c.includes(k)) isMum = true;
      for (const k of bengaluruKeywords) if (c.includes(k)) isBng = true;
      if (isMum && isBng) {
  ...
  for (const [c, cost] of costs.entries()) {
      if (cost >= 930 && cost <= 940) {
          console.log(`${c}: ₹${cost.toFixed(2)}`);
      }
  }

  console.log("=== HYD NEAR 3844 ===");
  for (const [c, cost] of costs.entries()) {
