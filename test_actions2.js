...
(async () => {
  for (const p of periods) {
    ...
    for (const row of data) {
      ...
      const val = parseInt(actions.find(a => a.action_type === 'offsite_conversion.custom.489677281790128')?.value || '0', 10);
      totalWalkin += val;
    }
    console.log(`Month ${p.since}: Total Walkins = ${totalWalkin}`);
  }
})();
