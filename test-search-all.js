...
async function main() {
  const url = `${BASE}/${accountId}/insights?fields=campaign_name,adset_name,date_start,spend,actions&time_increment=monthly&level=adset&limit=500&access_token=${access_token}`;
  const rows = await fetchAllPages(url);

  let keywords = ['elite', 'foot', 'fm', 'desk'];
  let found = new Set();
  ...
  for (const r of rows) {
    ...
    for (const kw of keywords) {
      if (cn.includes(kw) || an.includes(kw)) {
        found.add(`[${kw}] ${r.date_start} | C: ${r.campaign_name} | A: ${r.adset_name}`);
      }
    }
  }
