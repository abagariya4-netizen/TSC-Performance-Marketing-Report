...
async function main() {
  ...
  let funnels = { 'Top': 0, 'Mid': 0, 'Bot': 0, 'Group': 0, 'Growth': 0, 'Unknown': 0 };
  let total = 0;
  ...
  for (const r of rows) {
    ...
    // Explicitly exclude Growth campaigns like before? Let's check with and without
    if (cn.includes('growth')) continue; // Exclude to see if it matches Excel

    if (!matchesCategoryForMetrics(r.campaign_name, r.adset_name, 'Chair')) continue;
